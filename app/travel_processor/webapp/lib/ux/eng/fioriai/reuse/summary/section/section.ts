...
import { getModelVersion } from "../../common/model";
import { v2Model } from "../../common/types";
import type { Direction, ElementsWithHeadings, SectionTree } from "../types";
...
function getSections(objectPage: ObjectPageLayout): ObjectPageSection[] {
    ...
    return objectPage.getUseIconTabBar()
        ? sections.filter((section) => section.getId() === objectPage.getSelectedSection())
        : sections.filter((section) => section.getVisible());
}
...
function getHeadingsFromDomRef(domRef: Element, selectors: string[]): Element[] | undefined {
    ...
    for (const selector of selectors) {
        if (domRef.querySelectorAll(selector)?.length) {
            Array.from(domRef.querySelectorAll(selector)).forEach((header: Element) => headers.push(header));
        }
    }
 ...
 */
function getElementsForHeadings(subSectionHeadings: Element[], subSectionDomRef: Element, nodes: SectionTree[]) {
    const selectors = ["::Table", "::LineItem", "::Chart"];
    const skipElementsWithClass = [
    ...
    const filteredHeadings = subSectionHeadings.filter(
        (heading) =>
            (heading as HTMLElement).innerText.length > 0 &&
            !selectors.some(
                (selector) =>
                    heading.id.includes(selector) &&
                    !heading.id.includes(selector + "-title") &&
                    !heading.id.includes(selector + "-header")
            ) &&
            selectors.some((selector) => !heading.id.includes(selector))
    );

    const subSectionElements: ElementsWithHeadings[] = [];
    for (const heading of filteredHeadings) {
        const headingText = (heading as HTMLElement).innerText;
        let matchedSelector;
        let parentElement = findTopmostParentByIdPrefixSeparator(heading.parentNode as HTMLElement);
        while (
            parentElement &&
            (!parentElement.id ||
                parentElement.id.endsWith("-header") ||
                Array.from(parentElement.classList).some((cls) => skipElementsWithClass.includes(cls)))
        ) {
        ...
        if (matchedSelector) {
            ...
            const rootElement = subSectionDomRef.querySelector(
                "[id$='" + parentElement.id.slice(0, index + matchedSelector.length) + "']"
            );
            if (rootElement) {
                subSectionElements.push({ element: rootElement, heading: headingText });
            }
        }
        //no table or chart found in section -> form, identification or custom section
        else {
            subSectionElements.push({ element: parentElement, heading: headingText });
        }
    }
 ...
 */
function getSubSectionHeaders(subSection: ObjectPageSubSection, nodes: SectionTree[]) {
    ...
    const subSectionHeadings =
        getHeadingsFromDomRef(domRef, ["div[id$='-headerTitle']"]) ||
        getHeadingsFromDomRef(domRef, ["H1", "H2", "H3", "H4", "H5", "H6"]);
 ...
 */
function getHeadingPlaceholderFromDomRef(domRef: Element, selectors: { key: string; value: string }[]): string {
    for (const selector of selectors) {
...
function getSectionTitle(section: ObjectPageSection): string {
    ...
    if (headerName.length === 0) {
        ...
        const headings =
            getHeadingsFromDomRef(domRef, ["div[id$='-headerTitle']"]) ||
            getHeadingsFromDomRef(domRef, ["H1", "H2", "H3"]);
        const heading = headings?.find((heading) => (heading as HTMLElement).innerText.length > 0);
        headerName = heading
            ? (heading as HTMLElement).innerText
            : getHeadingPlaceholderFromDomRef(domRef, [
                  { key: "Table", value: "div[id*='::table']" },
                  { key: "Table", value: "div[id*='::Table']" },
                  { key: "Form", value: "div[id*='::Form']" },
                  { key: "Chart", value: "div[id*='::Chart']" },
                  { key: "Custom", value: "div[id*='::CustomSection']" }
              ]);
    }
    return headerName;
}
 ...
 */
export function createSectionTree(view: View): { sectionTree: SectionTree[]; selectCount: number; totalCount: number } {
    const sectionTree: SectionTree[] = [];
    const sections = getSections(view.getContent()[0] as ObjectPageLayout);
    let totalCount = 0;
    ...
    return { sectionTree, selectCount, totalCount };
}
...
function setSelection(
    ...
    if (!direction || direction === "DOWN") {
        const nodes = model.getProperty(path + "/nodes") as SectionTree[];
        if (nodes.length > 0) {
    ...
    if (!treeItem.isTopLevel() && (!direction || direction === "UP")) {
        ...
        if (parent) {
            const parentPath = (parent.getItemNodeContext() as { context: { sPath: string } }).context.sPath;
            ...
            if (!selected && parent.getSelected()) {
                ...
                const parentNodes = model.getProperty(parentPath + "/nodes") as SectionTree[];
                setSelection(
                    model,
                    parent,
                    parentPath,
                    parentNodes?.every((node) => node.selected === true),
                    "UP"
                );
            }
        }
 ...
 * @param model The JSONModel instance.
 */
function setSelectionState(model: JSONModel) {
    const sectionTree = model?.getProperty("/sections/sectionTree") as SectionTree[];
    let selectCount = 0;
    ...
    if (allSelected) {
        ...
        const partiallySelected = sectionTree.some(
            (section) => section.selected || section.nodes.some((node) => node.selected)
        );
 ...
 * @param model The JSONModel instance.
 */
export function selectionChange(selectedItems: TreeItemBase[], model: JSONModel) {
    for (const treeItem of selectedItems) {
        setSelection(
            model,
            treeItem,
            (treeItem.getItemNodeContext() as { context: { sPath: string } }).context.sPath,
 ...
 * @param model The JSONModel instance.
 */
export function selectAllSections(event: CoreEvent<{ selected: boolean }>, model: JSONModel) {
    const allSelected = event.getParameter("selected");
    const sectionTree = model.getProperty("/sections/sectionTree") as SectionTree[];
    const treeItem = (UI5Element.getElementById("SectionsTree") as Tree).getItems()[0] as TreeItemBase;
 ...
 */
export function getSelectedSections(model: JSONModel, selectionState: boolean, withNodes?: boolean): SectionTree[] {
    //get sections for requested selection state by checking on property selected in the sectionTree model
    const sectionTree = model.getProperty("/sections/sectionTree") as SectionTree[];
    const filteredSections: SectionTree[] = [];
 ...
 */
function processLazyLoadSections(
    view: View,
    lazyLoadSections: ObjectPageSubSection[],
    model: JSONModel,
    fnResolve: () => void,
    checkUIUpdated: (event: CoreEvent) => void
) {
    let bindingRefreshed = false;
    const smartTables: SmartTable[] = [];
    const oDataModel = view.getModel() as Model;
    const opLayout = view.getContent()[0] as ObjectPageLayout;
            ...
            blocks.forEach((block) => {
                ...
                if (Array.isArray(content)) {
                    for (const control of content) {
                        if (control.isA<SmartTable>("sap.ui.comp.smarttable.SmartTable")) {
 ...
 */
export async function unstashSections(model: JSONModel) {
    model.setProperty("/sections/busyState", true);
    const view = model.getProperty("/view") as View;
    const opSections = getSections(view.getContent()[0] as ObjectPageLayout);
    const lazyLoadSections: ObjectPageSubSection[] = [];
    ...
    return new Promise<void>((fnResolve, fnReject) => {
        setTimeout(() => {
            const checkUIUpdated = (event: CoreEvent) => {
                if (Rendering.isPending() || (event && event.getId().includes("beforeRebindTable"))) {
                    ...
                    const observer = new MutationObserver(() => {
                        ...
                        let busyIndicatorIds = Array.from(document.querySelectorAll('[id*="busyIndicator"]')).map(
