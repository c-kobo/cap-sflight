 ...
 * @returns Parsed value as a Date object, or undefined if the value is not a valid date
 */
function dateTime(value: boolean | string | number) {
    if (typeof value === "boolean" || value === "") {
        return undefined;
    }
...
export type ParsedValue = string | number | boolean | Date;
...
export default function parseValue(
    ...
    switch (edmType) {
        ...
        case "Edm.Double": // Floating-point number with 15 digits precision
            return float(value);
        ...
        case "Edm.TimeOfDay": // Clock time 00:00:00.0000000 through 23:59:59.9999999
            return time(value);

        case "Edm.String": // Fixed-length or variable-length sequence of UTF-8 characters
        default: // everything else is treated as string
            return String(value);
    }
}
