 ...
 */
export function createFieldMappingWithSimplifiedKeys<T>(
    metadata: EasyFilterMetadata,
    fieldTransformer: (field: PropertyMetadata) => T
): FieldMapping<T> {
    const fieldsByKey: Record<string, T> = {};
    const originalFieldNames: Record<string, string> = {};
    ...
    return { fieldsByKey, originalFieldNames };
}
