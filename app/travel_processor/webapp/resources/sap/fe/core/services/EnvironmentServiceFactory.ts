		...
		for (const [featureName, feature] of Object.entries(OptionalFeatures)) {
			...
			if (navigationService) {
				try {
					...
					const url =
						(feature.intent === "IntelligentPrompt-summarize" && !document.baseURI.startsWith("http://localhost"))
							? "/6df528df-33f5-44e8-bf43-408775134337.sapfecapsflight.sapfecaptravel/~3dd1c0e7-00cc-40b5-b948-ee439e69686b~/lib/ux/eng/fioriai/reuse/"
							: (await navigationService.resolveIntent(`#${feature.intent}`))?.url;
